...
Customer-facing 화면은 `모델 공통`, `선택 연식 기준`, `fallback`처럼 데이터 소유권이나 처리 방식을 설명하는 작업자용 문구를 노출하지 않는다. 데이터 출처와 fallback은 내부에서 정확히 적용하되 고객에게는 현재 선택한 모델·연식의 정보로 자연스럽게 표시한다.

## Bike Selection

Brand → Model → Model Year 순차 선택. 이전 단계가 결정되기 전 다음 단계가 시각적으로 경쟁하지 않게 한다. 브랜드는 인지 가능한 로고+텍스트를 활용하고, 모델은 수량이 많으면 검색을 제공한다. 추천/인기순은 명시적 정책 없이는 적용하지 않는다. Bike Selector에는 현재 단계의 선택 데이터만 제공하며 별도의 전체 모델 디렉터리를 함께 노출하지 않는다. 모델 목록은 작은 내부 스크롤 영역으로 제한하지 않고 페이지 스크롤을 사용한다. 브랜드 선택 이후의 이전·조회 Action은 모바일 안전영역을 고려한 고정 하단 Action Bar로 제공하고, 본문에는 Bar 높이만큼 충분한 하단 여백을 확보한다.
