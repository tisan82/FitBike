...
Sitemap은 `/`, `/bike-selector`, 활성 Model-Year URL을 포함한다. Model-Year는 실제 `updated_at`을 `lastModified`로 사용하며 요청 시각을 임의로 사용하지 않는다. 현재 규모에서는 단일 sitemap을 사용하고 존재하지 않거나 비활성인 ID를 생성하지 않는다.
...
대표 이미지는 Brand/Model/Year 기반의 간결한 alt를 사용한다. Placeholder를 실제 모델 이미지로 표현하지 않는다. OG image에는 absolute public URL만 사용한다. Model-Year 연식 링크는 crawler가 탐색 가능한 내부 링크다. 실제 Brand/Model directory가 없으면 SEO용 빈 page를 만들지 않는다.
