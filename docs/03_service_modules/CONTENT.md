...
| `HOW_TO` | 실제 작업 수행 | 준비, 단계, 완료 확인, 전문 점검 전환 기준 |
...
| `CHECKLIST` | 상황별 빠른 확인 | 확인 목록과 운행·점검의 다음 행동 |
...
5. 작업 공간이 좁거나 공식 분리·복구 절차가 없을 때 전문 점검으로 전환할 기준
...
점검 안내의 기본 목적은 사용자가 상태를 확인하고 다음 행동을 선택하게 하는 것이다.
따라서 일반적인 확인 항목에 `중단`, `STOP`, `즉시 정비소`를 반복하지 않는다.

- 일반 상태: `추가 확인`, `조정·정비 필요`, `운행 전 전문 점검 권장`처럼 다음 행동을 구체적으로 쓴다.
...
- 작성 근거는 본문 마지막의 `참고 공식 자료` 영역 하나로 통합하고 기본 접힘 상태로 제공한다.
- 펼친 화면에는 자료명만 표시하고 URL 문자열은 직접 노출하지 않는다. 공식 URL이 확인된 자료는 자료명 자체에 링크를 연결하며, URL이 확인되지 않은 자료는 명칭만 표시한다.
- 라이더 경험은 별도 후기 목록으로 노출하지 않는다. 콘텐츠 이해에 도움이 될 때만 실제 상황을 짧은 문단으로 자연스럽게 포함하며, 모든 콘텐츠에 같은 제목과 위치를 강제하지 않는다.
- 라이더 경험의 개별 출처·계정·URL은 서비스와 Source 원장에 저장하지 않는다. 수치, 원인, 모델 절차, 안전 판단은 별도 공식·기술 근거로 검증한다.
- Footer: 현재 콘텐츠를 제외한 연관 가이드 최대 6건과 `/contents` 전체 보기 경로를 제공한다. 제목의 핵심 주제 일치와 동일 Content Type을 우선하되 인기·추천 순위로 표현하지 않는다.
- 연관 가이드는 모바일에서 좌우 스와이프, 태블릿·PC에서 카드 그리드로 제공한다. 첫·마지막 카드에는 본문과 구분되는 좌우 여백을 유지한다.
