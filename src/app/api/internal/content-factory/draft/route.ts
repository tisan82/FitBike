...
import { ContentFactoryValidationError, storeBlockedContentFactory } from "@/services/content-factory.service";
...
export async function POST(request: Request) {
  ...
  if (!payload?.success) return errorResponse("VALIDATION_ERROR", "비공개 초안 저장 요청이 올바르지 않습니다.", 422);
  try {
    return successResponse(await storeBlockedContentFactory(payload.data), 201);
  } catch (error) {
    if (error instanceof ContentFactoryValidationError) return errorResponse("VALIDATION_ERROR", error.message, 422);
    console.error("Content Factory blocked draft store failed", error);
    return errorResponse("DRAFT_CONFLICT", "초안 저장 조건 또는 큐 상태를 확인해 주세요.", 409);
  }
}
