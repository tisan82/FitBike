import { z } from "zod";

import { errorResponse, successResponse } from "@/lib/api/response";
import {
  BatteryProductNotFoundError,
  getBatteryProductDetail,
} from "@/services/battery-detail.service";

const paramsSchema = z.object({
  batteryProductId: z.coerce.number().int().positive(),
});

type RouteContext = {
  params: Promise<{ batteryProductId: string }>;
};

export async function GET(_request: Request, context: RouteContext) {
  const parsed = paramsSchema.safeParse(await context.params);

  if (!parsed.success) {
    return errorResponse(
      "VALIDATION_ERROR",
      "올바른 배터리 상품 ID가 필요합니다.",
      422,
    );
  }

  try {
    return successResponse(
      await getBatteryProductDetail(parsed.data.batteryProductId),
    );
  } catch (error) {
    if (error instanceof BatteryProductNotFoundError) {
      return errorResponse("NOT_FOUND", error.message, 404);
    }

    console.error("Failed to load battery product detail", error);
    return errorResponse(
      "INTERNAL_ERROR",
      "배터리 상품 정보를 불러오지 못했습니다.",
      500,
    );
  }
}
