  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    for (const field of config.fields) {
      ...
      if (field.type === "select" && raw !== "" && !field.options?.some((option) => option.value === raw)) {
        setMessage(`${field.label}: 허용되지 않은 값입니다.`);
        return;
      }
