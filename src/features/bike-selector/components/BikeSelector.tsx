...
import { useBrandOptionsQuery, useModelOptionsQuery, useYearOptionsQuery } from "@/features/bike-selector/hooks/useBikeSelectorQueries";
...
export function BikeSelector() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const brand = brands.data?.find((item) => item.brandId === selector.selectedBrandId);
    const model = models.data?.find((item) => item.bikeModelId === selector.selectedModelId);
    const year = years.data?.find((item) => item.bikeModelYearId === selector.selectedModelYearId);
    if (brand && model && year) {
      const stored: StoredBike = {
        bikeModelYearId: year.bikeModelYearId,
        brandName: brand.brandNameKo ?? brand.brandNameEn,
  ...
  const handlePrevious = () => {
    if (selector.selectedModelId !== null) { selector.selectModel(null); return; }
    if (selector.selectedBrandId !== null) selector.selectBrand(null);
  };
      ...
      <div className="px-5 py-5 sm:py-8">
        <div className="mx-auto max-w-2xl">
          <form className="space-y-8" onSubmit={handleSubmit}>
            <section>
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-foreground">{currentStep === 1 ? "1단계. 브랜드 선택" : "선택된 브랜드"}</h2>
                <p className="mt-2 text-base leading-6 text-foreground-secondary">{currentStep === 1 ? "내 바이크의 브랜드를 선택하세요." : (() => { const brand = brands.data?.find((item) => item.brandId === selector.selectedBrandId); return brand ? (brand.brandNameKo ?? brand.brandNameEn) : ""; })()}</p>
              </div>
              <BrandSelect brands={brands.data ?? []} value={selector.selectedBrandId} loading={brands.isLoading} onChange={selector.selectBrand} selectedOnly={currentStep !== 1} />
            </section>

            {selector.selectedBrandId && currentStep >= 2 && (
              <section>
                <div className="mb-6">
                  <h2 className="text-2xl font-bold text-foreground">{currentStep === 2 ? "2단계. 모델 선택" : "선택된 모델"}</h2>
                  <p className="mt-2 text-base leading-6 text-foreground-secondary">{currentStep === 2 ? "선택한 브랜드의 모델을 선택하세요." : (() => { const model = models.data?.find((item) => item.bikeModelId === selector.selectedModelId); return model ? (model.modelNameKo ?? model.modelNameEn) : ""; })()}</p>
                </div>
                <ModelSelect models={models.data ?? []} value={selector.selectedModelId} disabled={selector.selectedBrandId === null} loading={models.isFetching} onChange={selector.selectModel} selectedOnly={currentStep !== 2} />
              </section>
            )}

            {selector.selectedModelId && currentStep >= 3 && (
              <section>
                <div className="mb-6"><h2 className="text-2xl font-bold text-foreground">3단계. 연식 선택</h2><p className="mt-2 text-base leading-6 text-foreground-secondary">선택한 모델의 연식을 선택하세요.</p></div>
