        ...
        switch (block.type) {
          case "heading": {
            ...
            return block.level === 2 ? (
              <h2 className="border-t border-border pt-9 text-xl font-bold leading-8 first:border-t-0 first:pt-0 sm:text-2xl" key={key}>{block.text}</h2>
            ) : (
              <h3 className="pt-3 text-lg font-bold leading-8 sm:text-xl" key={key}>{block.text}</h3>
            );
          }
          case "paragraph":
            return block.text?.trim() ? <p className="whitespace-pre-line text-pretty" key={key}>{block.text}</p> : null;
          case "image": {
            ...
            return src ? (
              <figure className="space-y-3 py-2" key={key}>
                ...
                {block.caption?.trim() ? <figcaption className={isMobileGuide ? "mx-auto max-w-md text-sm leading-6 text-foreground-secondary" : "text-sm leading-6 text-foreground-secondary"}>{block.caption}</figcaption> : null}
              </figure>
            ) : null;
          }
          case "image_gallery": {
            const uniqueImages = block.images.filter((image) => {
              if (!image.storagePath?.trim() || seenImagePaths.has(image.storagePath)) return false;
              seenImagePaths.add(image.storagePath);
              return true;
            });
            ...
            return (
              <section aria-label="관련 이미지" className={`grid grid-cols-1 gap-4 py-2 sm:gap-5 ${block.columns === 3 ? "sm:grid-cols-2 lg:grid-cols-3" : "sm:grid-cols-2"}`} key={key}>
                {uniqueImages.map((image, imageIndex) => <ContentVisual compact image={image} key={`${image.storagePath}-${imageIndex}`} />)}
              </section>
            );
          }
          case "bullet_list":
            return block.items?.length ? <ul className="list-disc space-y-3 pl-6 marker:text-primary" key={key}>{block.items.filter(Boolean).map((item, itemIndex) => <li className="pl-1" key={itemIndex}>{item}</li>)}</ul> : null;
          case "numbered_list":
            return block.items?.length ? <ol className="list-decimal space-y-3 pl-6 marker:font-bold marker:text-primary" key={key}>{block.items.filter(Boolean).map((item, itemIndex) => <li className="pl-1" key={itemIndex}>{item}</li>)}</ol> : null;
          case "step":
            return block.body?.trim() ? <section className="rounded-2xl border border-border bg-surface p-5 sm:p-6" key={key}><h3 className="text-lg font-bold leading-7">{block.number ? `${block.number}. ` : ""}{block.title}</h3><p className="mt-3 whitespace-pre-line leading-8">{block.body}</p></section> : null;
          case "tip":
          case "warning":
            return block.body?.trim() ? <aside className={`rounded-2xl border p-5 sm:p-6 ${block.type === "tip" ? "border-selected-border bg-selected-background" : "border-amber-300 bg-amber-50"}`} key={key}><p className="font-bold">{block.title ?? (block.type === "tip" ? "알아두면 좋은 점" : "주의")}</p><p className="mt-3 whitespace-pre-line leading-8">{block.body}</p></aside> : null;
          case "table":
            return block.rows?.length ? <div className="max-w-full overflow-x-auto rounded-2xl border border-border" key={key}><table className="w-full min-w-lg border-collapse text-left text-base leading-7"><thead className="bg-surface-secondary"><tr>{block.headers.map((header, cellIndex) => <th className="border-b border-border px-4 py-3 font-bold" key={cellIndex} scope="col">{header}</th>)}</tr></thead><tbody>{block.rows.map((row, rowIndex) => <tr className="border-b border-border last:border-b-0" key={rowIndex}>{row.map((cell, cellIndex) => <td className="px-4 py-3 align-top" key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div> : null;
        }
      })}
    </div>
  );
