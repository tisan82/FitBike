      ...
      {blocks.map((block, index) => {
        ...
        switch (block.type) {
          ...
          case "image_gallery": {
            ...
            if (block.layout === "swipe") return <section aria-label="작업 및 확인 이미지" className="flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-3 pt-2 scroll-px-4 sm:px-5 sm:scroll-px-5" key={key}>{uniqueImages.map((image, imageIndex) => <div className="w-[84%] min-w-[84%] snap-start sm:w-[46%] sm:min-w-[46%] lg:w-[32%] lg:min-w-[32%]" key={`${image.storagePath}-${imageIndex}`}><ContentVisual compact contain image={image} /></div>)}</section>;
