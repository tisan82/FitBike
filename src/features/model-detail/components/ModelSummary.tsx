...
export function ModelSummary({ model }: { model: ModelDetailData }) {
  ...
  const engineCc = model.engineCc ?? model.modelEngineCc;
  const category = model.category ?? model.modelCategory;
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
      ...
      <div className="space-y-5 p-5 sm:p-7">
        ...
        <dl className="grid grid-cols-2 gap-5 sm:grid-cols-4">
          <Item label="배기량" value={engineCc === null ? null : `${Math.round(engineCc)}cc`} />
          <Item label="카테고리" value={category} />
